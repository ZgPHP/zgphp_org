            ...
            if (count($r2) > 0) {
                foreach ((array)$r2 as $r3) {
                    if(@$r3['value'] != '' && strlen($r3['name']) != 1 && $r3['type'] != 'select' && $r3['type'] != 'radio' && $r3['type'] != 'checkbox'){
                        update_option('Inkland_'.$r1['id'].'_'.$r3['name'], $r3['value']);
                    }
                }
            }       
        }
...
function tk_settings_page_init() {
    ...
    for ($i = 0; $i <= count($pages) - 1; $i++) {
        if ($i == 0) {
            $settings_page .= add_menu_page($pages[$i]['slug'], 'Inkland', 'edit_theme_options', $pages[$i]['slug'], 'tk_settings_page');
...
function tk_save_theme_settings() {
    ...
    if ($pagenow == 'admin.php' && isset($_GET['page'])) {
        if (isset($tab)) {
            ...
            foreach ($_POST as $var => $value) {
                if ($var != '_wpnonce' && $var != '_wp_http_referer' && $var != 'Submit' && $var != 'ilc-settings-submit' && !preg_match("/_required/i", $var)) {//$_POST variables which we don't want to save as options
                    update_option('Inkland_'.$tab . '_' . $var, $value);
                }
            }
        }
    }
                                ...
                                if ($row_items == 4) {
                                    if (count($r2) > 0) {
                                        ...
                                        foreach ($r2 as $r3) {
                                            ...
                                            if(isset($_GET['dev'])){
                                                $dev = '<br /><font color="red">'.'Inkland_'.$tab . '_' . $r3['name'].'</font>';
                                            }else{
                                                $dev = '';
                                            }
                                            ...
                                            if ($r3['type'] == 'text') {//TYPE: TEXT
                                                ...
                                                if (get_option('Inkland_'.$tab . '_' . $r3['name']) != '') {
                                                    $val = get_option('Inkland_'.$tab . '_' . $r3['name']);
                                            ...
                                            if ($r3['type'] == 'checkbox') {//TYPE: CHECKBOX
                                                if (get_option('Inkland_'.$tab . '_' . $r3['name']) != '') {
                                                    $val = get_option('Inkland_'.$tab . '_' . $r3['name']);
                                                    @$val_database = get_option('Inkland_'.$tab . '_' . $r3['name']);
                                                } else {
                                                    $val = $r3['value'];
                                                    @$val_database = array();
                                                }
